...
import { hashDeviceKey } from '../src/utils/device-key';
...
async function main() {
  ...
  // ---------------------------------------------------------
  // 1. TURNOS (Especificados com precisão matemática em minutos)
  // ---------------------------------------------------------
  console.log('⏳ Criando Turnos de Alunos...');
  const turnoAlunoManha = await prisma.turno.upsert({
    where: { id: 'aluno-manha' },
    update: {}, create: { id: 'aluno-manha', nome: 'Aluno - Manhã', horaInicio: 480, horaFim: 720, diasSemana: [1, 2, 3, 4, 5] }, // 08:00 às 12:00
  });
  const turnoAlunoTarde = await prisma.turno.upsert({
    where: { id: 'aluno-tarde' },
    update: {}, create: { id: 'aluno-tarde', nome: 'Aluno - Tarde', horaInicio: 780, horaFim: 1020, diasSemana: [1, 2, 3, 4, 5] }, // 13:00 às 17:00
  });
  const turnoAlunoNoite = await prisma.turno.upsert({
    where: { id: 'aluno-noite' },
    update: {}, create: { id: 'aluno-noite', nome: 'Aluno - Noite', horaInicio: 1080, horaFim: 1320, diasSemana: [1, 2, 3, 4, 5] }, // 18:00 às 22:00
  });

  console.log('⏳ Criando Turnos de Professores (com margem de 30min)...');
  const turnoProfManha = await prisma.turno.upsert({
    where: { id: 'prof-manha' },
    update: {}, create: { id: 'prof-manha', nome: 'Professor - Manhã', horaInicio: 450, horaFim: 750, diasSemana: [1, 2, 3, 4, 5] }, // 07:30 às 12:30
  });
  const turnoProfNoite = await prisma.turno.upsert({
    where: { id: 'prof-noite' },
    update: {}, create: { id: 'prof-noite', nome: 'Professor - Noite', horaInicio: 1050, horaFim: 1350, diasSemana: [1, 2, 3, 4, 5] }, // 17:30 às 22:30
  });

  console.log('⏳ Criando Turnos de Staff e Gestão...');
  const turnoComercial = await prisma.turno.upsert({
    where: { id: 'staff-comercial' },
    update: {}, create: { id: 'staff-comercial', nome: 'Funcionário - Comercial', horaInicio: 420, horaFim: 1080, diasSemana: [1, 2, 3, 4, 5] }, // 07:00 às 18:00
  });
  const turnoIntegral = await prisma.turno.upsert({
    where: { id: 'gestao-integral' },
    update: {}, create: { id: 'gestao-integral', nome: 'Gestor - Integral', horaInicio: 360, horaFim: 1439, diasSemana: [1, 2, 3, 4, 5, 6] }, // 06:00 às 23:59
  });
  ...
  const portaria = await prisma.sala.upsert({
    ...
    update: {}, create: { nome: 'Portaria Principal', bloco: 'Entrada' },
  });

  const lab101 = await prisma.sala.upsert({
    where: { nome_bloco: { nome: 'Laboratório 101', bloco: 'A' } },
    update: {}, create: { nome: 'Laboratório 101', bloco: 'A' },
  });
  ...
  // 4. USUÁRIOS
  // ---------------------------------------------------------
  console.log('👥 Criando Usuários (Gestor, Recepcionista, Professor, Aluno)...');
  ...
  // ---------------------------------------------------------
  // 5. LOGS DE ACESSO (Mock para Relatórios e Dashboards)
  // ---------------------------------------------------------
  ...
  if (totalLogs === 0) {
    await prisma.logAcesso.createMany({
      data: [
        // 1. Acesso Padrão Concedido (Catraca - Entrada)
        {
          ...
          dataHora: new Date(Date.now() - 1000 * 60 * 60 * 24), // Ontem
        },
        // 2. Acesso Padrão Concedido (Laboratório)
        {
          status: StatusAcesso.CONCEDIDO,
        ...
        // 3. Acesso Negado (Hard Block) - Aluno tentando entrar no Lab fora do horário
        {
        ...
        // 4. Soft Audit - Acesso Concedido fora de hora na Catraca Principal
        {
          status: StatusAcesso.CONCEDIDO,
          finalidade: FinalidadeLog.ENTRADA_PREDIO,
          direcao: DirecaoAcesso.ENTRADA,
          ...
          uidCartao: aluno.cartaoId,
          dispositivoId: catracaEntrada.id,
          dataHora: new Date(Date.now() - 1000 * 60 * 60 * 5), // 5 horas atrás
        },
        // 5. Acesso Negado - Invasão/Cartão Desconhecido
        {
          status: StatusAcesso.NEGADO,
          finalidade: FinalidadeLog.ENTRADA_PREDIO,
          direcao: DirecaoAcesso.ENTRADA,
